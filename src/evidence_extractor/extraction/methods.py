...
def extract_methods_and_quality(
    client: GeminiClient, text_snippet: str
) -> Optional[QualityScore]:
    if not client.is_configured():
        logger.warning(
            "Cannot extract methods/quality; Gemini client is not configured."
        )
        return None
    ...
    First, identify the primary study design (e.g., Randomized Controlled Trial,
    Cohort Study, Case-Control Study, Systematic Review, Case Report, etc.).

    Second, based on the study design, assign a quality score according to this
    rubric:
    ...
    3. "justification": A brief summary of the study design that justifies the
    score.
    ...
    try:
        ...
        logger.error(
            "Failed to decode JSON from Gemini for quality assessment. "
            f"Response was:\n{response_text}"
        )
        return None
