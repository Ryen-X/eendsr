...
def extract_claim_texts(client: GeminiClient, text_snippet: str) -> List[str]:
    if not client.is_configured():
        logger.warning("Cannot extract claims; Gemini client is not configured.")
        return []

    prompt = f"""
    Analyze the following text from a scientific research paper. Your task is to
    identify and extract the main factual claims or findings.
    A "claim" is a specific, verifiable statement about the results or conclusions
    of the study.
    ...
    Do NOT extract general statements, background information, or descriptions of
    methods.

    Return your findings as a single, valid JSON array of objects. Each object in
    the array should represent one claim and must have a single key: "claim_text".
    The value of "claim_text" should be the verbatim text of the claim as found in
    the paper.
    ...
    try:
        ...
        logger.info(
            f"Successfully extracted text for {len(extracted_claim_texts)} claims."
        )
        return extracted_claim_texts
    except json.JSONDecodeError:
        logger.error(
            "Failed to decode JSON from Gemini for claim extraction. "
            f"Response was:\n{response_text}"
        )
        return []
