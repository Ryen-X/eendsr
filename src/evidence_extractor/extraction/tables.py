...
def extract_tables_with_llm(doc: fitz.Document, client: GeminiClient) -> List[ExtractedTable]:
    ...
    for i, table_area in enumerate(all_tables):
        ...
        try:
            ...
            if data.get("structured_data"):
                provenance = Provenance(source_filename=doc.name, page_number=table_area.page, bounding_box=list(table_area._bbox))
                structured_table = ExtractedTable(summary=data.get("summary"), table_data=data["structured_data"], provenance=provenance)
            ...
            logger.error(f"Failed to parse Gemini's response for table on page {table_area.page}. Error: {e}")
