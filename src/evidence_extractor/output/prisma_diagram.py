...
def generate_prisma_diagram(extraction: ArticleExtraction, output_path: str):
    ...
    dot.attr('node', shape='box', style='rounded,filled', fillcolor='whitesmoke', fontname='helvetica')
    ...
    if n_excluded > 0:
        dot.node('excluded', f'Records excluded (n = {n_excluded})', fillcolor='lightcoral')
    dot.edge('identification', 'screening', arrowhead='none')
    if n_excluded > 0:
        dot.edge('screening', 'excluded', label='  Exclusion reason: N/A for single-file tool')
    ...
    try:
        ...
        logger.error(f"Failed to generate PRISMA diagram. Is Graphviz installed on your system? Error: {e}")