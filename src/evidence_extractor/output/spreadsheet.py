...
def export_to_excel(extraction: ArticleExtraction, output_path: str):
    ...
    if extraction.claims:
        ...
        ws_claims.append([
            "Claim Text", "Uncertainty Annotation", "Page Number", "Validation Status", "Reviewer Comment"
        ])
        for claim in extraction.claims:
            ws_claims.append([
                claim.claim_text, claim.uncertainty_annotation, claim.provenance.page_number,
                claim.correction_metadata.status.value, claim.correction_metadata.reviewer_comment
            ])
    for i, table in enumerate(extraction.tables):
        sheet_title = _clean_sheet_title(f"Table {i+1} (Page {table.provenance.page_number})")
